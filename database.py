...
def getDatabase():
    ...
    user_input = input("Please enter the location of the question database or press backspace: %s"%default + chr(8)*-1)
    if not user_input:
        user_input = default
...
if not path.exists("defaultHistory.db"):
    ...
    question = '''INSERT INTO questions(question_id, question, false_answers, answer_id, points)
              VALUES(1, 'Which queen had the shortest reign of Henry VIII’s six wives?', 1, 1, 100),
                ...
                (21, 'What was Mary Anning (1799–1847) famous for?', 3, 3, 500),
                (22, 'Who gave Queen Elizabeth I the soubriquet ‘Gloriana’?', 1, 1, 500),
                (23, 'Although never taking her seat, who was the first woman to be elected to the houses of parliament?', 2, 2, 500),
                (24, 'Where was Napoleon Bonaparte born?', 3, 3, 500),
                (25, 'Can you name the five beach codenames used by Allied forces on D-Day?', 1, 1, 500),
                (26, 'Where was the first British colony in the Americas?', 2, 2, 500),
                (27, 'In August 1819, around 60,000 peaceful pro-democracy protestors were attacked in an open square in Manchester. This event was known as…', 3, 3, 500),
                (28, 'Which rock band formed in 1994 takes its name from a term used by the Allies in the Second World War to describe various UFOs?', 1, 1, 500),
                (29, 'In which year did Emily Wilding Davison die as a result of a collision with King George V’s horse during the Epsom Derby?', 2, 2, 500),
                (30, 'In medieval history, what was a ‘schiltron’?', 3, 3, 500) '''
